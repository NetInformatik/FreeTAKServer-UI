...
def index():
    ...
    websocketkey=app.config['WEBSOCKETKEY'], apikey=app.config['APIKEY'], port=app.config['PORT'], ip=app.config['IP'], ipclient=app.config['IP_CLIENT'],
...
def webmapApi():
    return render_template('webmap.html', segment="webmap", uiversion=app.config['UIVERSION'],
                           ...
                           ip=app.config['IP'], webmapip=app.config['WEBMAPIP'], webmapport=app.config["WEBMAPPORT"], ipclient=app.config['IP_CLIENT'])
...
def page_user():
    ...
    if "update" in request.form:
        ...
        return render_template('page-user.html', form=update_account_form, ip=app.config["IP"], ipclient=app.config['IP_CLIENT'], port=app.config["PORT"], websocketkey=app.config['WEBSOCKETKEY'], apikey=app.config['APIKEY'], user_id=uid)
